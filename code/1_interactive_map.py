...
from commons import get_street_coords, YEAR
...
if __name__ == '__main__':
    ...
    for i, street in tqdm(list(enumerate(streets))):
        loc = get_street_coords(street.upper())
    ...
    dt_index_epochs = datetime_index.astype('int64') // 10 ** 9 #neu 2025
    ...
    g = TimeSliderChoropleth(
        name=f'Sperrmüllkarte Karlsruhe {YEAR}',
        data=data,
        styledict=styledict,
    ).add_to(m)
...
html = html.replace('<div class="folium-map"',
                    f'''<b>Sperrmüllkarte Karlsruhe {YEAR}</b><br>
